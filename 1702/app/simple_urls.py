...
def task_detailed(task_id: int):
    ...
    if not current_task.is_active and current_user.user_role is UserType.STUDENT:
        logger.warning(f"Student {current_user.username} with id: {current_user.user_id} is trying to access task {current_task.task_id}: {current_task.task_name}")
        abort(404)
    ...
    if request.method == 'POST':
        ...
        if not file or not file.filename.endswith('.c'):
            ...
            return redirect(url_for('simple_routes.task_detailed', task_id=task_id))
...
def submission_status(submission_id):
    ...
    if sub is None:
        abort(404)
    if sub.user_id != current_user.user_id and current_user.user_role not in (UserType.TEACHER, UserType.ADMIN):
        logger.warning(f"User {current_user.username} with id: {current_user.user_id} is trying to get access to forbidden submission {sub.submission_id}")
        abort(404)
...
@login_required
def submission_detailed(submission_id: int):
    current_submission = db.session.get(Submission, submission_id)

    if current_submission is None:
        abort(404)

    if current_submission.user_id != current_user.user_id and current_user.user_role not in (UserType.TEACHER, UserType.ADMIN):
        logger.warning(f"User {current_user.username} with id: {current_user.user_id} is trying to get access to forbidden submission {current_submission.submission_id}")
        abort(404)
